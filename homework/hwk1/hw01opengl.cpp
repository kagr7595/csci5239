...
void Hw01opengl::initializeGL()
{
   ...
   //  Build shader
   if (!shader.addShaderFromSourceFile(QOpenGLShader::Vertex,":/hw01.vert"))
      Fatal("Error compiling hw01.vert\n"+shader.log());
   if (!shader.addShaderFromSourceFile(QOpenGLShader::Fragment,":/hw01.frag"))
      Fatal("Error compiling hw01.frag\n"+shader.log());
   if (!shader.link())
      Fatal("Error linking shader\n"+shader.log());

   // Cube
   objects.push_back(new Cube(":pi.png"));

   // Icosahedron
   objects.push_back(new Icosahedron(":pi.png"));
...
void Hw01opengl::paintGL()
{
   ...
   {
      shader.bind();
      QVector3D window_dim = QVector3D(width(),height(),1.0);
      shader.setUniformValue("window_dim",window_dim);
   }

   //  Draw scene
   // Translate object dependent on x/y/z slider positions
   glPushMatrix();
   int max_pos = 10+10; //larger than max slider size so that it doesn't cut out in the xyz extremes
   glTranslated(x_pos*dim/max_pos,y_pos*dim/max_pos,z_pos*dim/max_pos);
   if (obj) obj->display();
   glPopMatrix();
